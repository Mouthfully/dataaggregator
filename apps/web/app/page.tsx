...
import { ENVELOPE_FIELDS, SPINE, claim, optionalClaim, productName } from "./_content";

/**
 * The marketing site's first screen.
 *
 * EVERY SENTENCE HERE COMES FROM the strict or optional claim resolver. Structural words --
 * headings, labels, the step names -- are the only prose written in this file, and a test asserts
 * that every claim-shaped string in the rendered HTML traces back to `@repo/brand`. See
 * `app/_content.ts` for why.
 *
 * `00-repo-map.md` section 7: roughly 40% of the artboard's copy survives section 11. What is here
 * is the subset of that 40% whose capabilities exist today. The competitors card, the 22-source
 * count, the write-side promises, "Frankfurt by default" and "pay as you go" are absent because the
 * claims list makes them unreachable, not because I remembered to leave them out.
 *
...
export default function Page() {
  ...
  const positioning = optionalClaim("positioning");
  const spine = SPINE.flatMap((item) => {
    const text = optionalClaim(item.claim);
    return text === null ? [] : [{ ...item, text }];
  });
  const optionalCorrectness = ["second-pass"].flatMap((id) => {
    const text = optionalClaim(id);
  ...
  const connectors = optionalClaim("connectors");
  const serp = optionalClaim("serp-bought");
  const pricing = optionalClaim("pricing-two-units");
  const billing = optionalClaim("billing-fairness");
  ...
  return (
    <main className="text-ink font-body">
      ...
      <section className="mx-auto max-w-5xl px-6 pt-24 pb-16 sm:pt-32">
        ...
        {positioning === null ? null : (
          <p className="text-ink-muted mt-6 max-w-2xl text-lg text-pretty">{positioning}</p>
        )}
        ...
        <div className="mx-auto grid max-w-5xl gap-px px-6 py-16 sm:grid-cols-2 lg:grid-cols-4">
          {spine.map(({ step, text }, index) => (
            <div key={step} className="pr-6">
              ...
              <p className="text-ink-muted mt-2 text-sm text-pretty">{text}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-surface border-line-soft border-y">
        <div className="mx-auto max-w-5xl px-6 py-20">
          ...
          <div className="mt-10 grid gap-8 sm:grid-cols-2">
            ...
            {optionalCorrectness.map((copy) => (
              <p key={copy} className="text-ink-muted text-sm text-pretty">
                {copy}
              </p>
            ))}
          </div>
      ...
      <section className="mx-auto max-w-5xl px-6 py-20">
        ...
        {/* No count, and no grid of twenty-two marks: the source list follows implementations, and
            the forbidden-claims list makes an arbitrary catalogue size unrenderable. */}
        {connectors === null ? null : (
          <p className="text-ink-muted mt-4 max-w-2xl text-pretty">{connectors}</p>
        )}
        {serp === null ? null : (
          <p className="text-ink-subtle mt-3 max-w-2xl text-sm text-pretty">{serp}</p>
        )}
        <p className="text-ink-muted mt-8 max-w-2xl text-pretty">{claim("one-shape")}</p>
      </section>

      {/* ------------------------------------------------------------- pricing */}
      {pricing === null && billing === null ? null : (
        <section className="border-line-soft border-t">
          <div className="mx-auto max-w-5xl px-6 py-20">
            <h2 className="font-display text-ink text-3xl tracking-tight">How it is priced</h2>
            {pricing === null ? null : (
              <p className="text-ink-muted mt-4 max-w-2xl text-pretty">{pricing}</p>
            )}
            {billing === null ? null : (
              <p className="text-ink-subtle mt-3 max-w-2xl text-sm text-pretty">{billing}</p>
            )}
          </div>
        </section>
      )}
