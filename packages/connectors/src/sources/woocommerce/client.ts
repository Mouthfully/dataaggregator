 ...
 * `modified_before` bounds are probably inclusive at both ends, and WHETHER they are is not
 * something this connector has verified against a live store (`34` §5). Splitting at
 * `[after, mid]` and `[mid + 1s, before]`
...
export async function probeStore(options: WooFetchOptions): Promise<WooProbe> {
  // NORMALISED HERE TOO, not merely assumed. A probe that took the caller's word for the store URL
  // would answer "permalinks are off" for a merchant who pasted a /wp-admin address, because the
  // path would be concatenated into the API route and 404. Re-running it is free and idempotent,
  // and it is what makes "validates the store URL" true rather than nearly true. The result is the
  // value the connect surface should store.
  const storeUrl = normaliseStoreUrl(options.storeUrl);
  ...
  try {
    ...
    throw probeFailure(error, failureBody, storeUrl);
  }

  const body = (await response.json().catch(() => null)) as unknown;
  if (!Array.isArray(body)) {
    throw new WooClientError(
      `woocommerce: ${storeUrl} answered the orders endpoint with 200 and something that ` +
  ...
  return { storeUrl, totalOrders: readPagination(response.headers, 1).totalOrders };
}
