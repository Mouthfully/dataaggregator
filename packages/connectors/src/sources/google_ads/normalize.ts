...
function readPath(row: GoogleAdsRow, path: string): unknown {
  ...
  for (const segment of path.split(".")) {
    ...
    node = camel in record ? record[camel] : record[segment];
  }
  return node;
}
...
export function normalizeGoogleAdsSearch(options: GoogleAdsNormalizeOptions): EnvelopeRow[] {
  ...
  const mask = new Set(
    response.fieldMask
      .split(",")
      .map((path) => toCamelPath(path.trim()))
      .filter((path) => path !== ""),
  );
  ...
  const requireMask = (path: string, code: GoogleAdsNormalizeErrorCode, why: string) => {
    if (!mask.has(toCamelPath(path))) {
      throw new GoogleAdsNormalizeError(`google_ads: the query did not select ${path}. ${why}`, code);
    }
  };
  ...
  for (const path of mask) {
    ...
    // Two mask paths that mean one canonical metric would write one key twice and the second would
    // silently win -- a number nothing in the response supports. It cannot happen with today's
    // one-to-one map, and it is one dictionary entry away from being able to: `all_conversions`
    // onto `conversions`, or a mask that carries both spellings of one field.
  ...
  for (const [index, row] of (response.results ?? []).entries()) {
    ...
    const parentId =
      spec.parentIdPath === null ? null : readRequiredString(row, spec.parentIdPath);
