...
function assertCallable(options: GoogleAdsClientOptions): void {
  ...
  if (
    ...
    !CUSTOMER_ID.test(options.loginCustomerId)
  ) {
    throw new GoogleAdsClientError(
...
export async function search(
  ...
  const token = typeof parsed.nextPageToken === "string" && parsed.nextPageToken !== ""
...
export async function* searchPages(
  ...
  for (;;) {
    ...
    const page = await search(
      { ...options, budget: { ...options.budget, consumedToday: options.budget.consumedToday + spent } },
      query,
      token,
    );
