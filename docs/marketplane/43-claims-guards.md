...
- **The `search_console` connector currently fails `check-capabilities.mjs`.** As of this writing
  `packages/connectors/src/sources/search_console/` holds both `client.ts` and `normalize.ts` while
  `IMPLEMENTED_SOURCE_IDS` names only `ga4` and `woocommerce`, so the existing capability guard
  exits 1. That is the guard working exactly as designed — a connector landing without its claim
  mirror — and it belongs to the connector change, not to this one. Untouched deliberately.
...
disjoint paths, and two gates are red from their in-flight work, not from this change.** Both are
named below rather than rounded to "pass".

| Gate | Result |
|---|---|
| `pnpm exec biome lint .` | **pass by exit code** (0) — 5 warnings, 1 info, all pre-existing and none in a file this PR touches. The new guards produce no diagnostic. |
| `pnpm exec biome format --write scripts/check-copy.mjs scripts/check-claim-sources.mjs package.json` | **pass** — 3 files checked, 2 reformatted (the two new guards). Only this PR's own files were passed to `--write`. |
| `pnpm exec biome format .` (read-only, whole repo) | **FAILS on four files, none of them this PR's**: `packages/connectors/src/sources/{search_console/client.ts, search_console/fixtures.ts, woocommerce/client.ts, woocommerce/paging.test.ts}` — the concurrent connectors workflow's in-flight edits. `pnpm exec biome format scripts/ package.json .github/` passes: 8 files, no fixes. |
| `pnpm -r typecheck` | **FAILS in `packages/connectors` only**: `src/sources/woocommerce/paging.test.ts(227,9): error TS2322` — again the concurrent workflow. Every other package passes, `apps/web` included. This PR adds no TypeScript. |
...
| `node scripts/check-capabilities.mjs` | **FAILS**: `implemented source "search_console" is absent from the connector-claim source list`. The concurrent connector landed `client.ts` + `normalize.ts` without updating `IMPLEMENTED_SOURCE_IDS`; `packages/brand/src/claims.ts` is byte-identical to how this PR found it (md5 `1d4ef828…`, verified before and after). Theirs to close. |
